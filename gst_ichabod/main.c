      ...
      char *argv[])
{
  ...
  GMainLoop *loop;
  GstElement *asource, *vsource, *aconv, *imgdec, *venc, *aenc, *mux, *sink;
  GstBus *bus;
  guint bus_watch_id;
  ...
  sink      = gst_element_factory_make ("filesink", "fink");

  if (!pipeline || !vsource || !imgdec || !venc || !mux || !sink) {
  ...
  gst_bin_add_many (GST_BIN (pipeline),
                    vsource, imgdec, venc, mux, sink, NULL);
  
  // link the elements together
  gst_element_link_many (vsource, imgdec, venc, mux, sink, NULL);
